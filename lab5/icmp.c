...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	for (int i = 0; i < count; i++){
		...
		*/
		...
		printf("icmp_seq=%d ttl=%d ", icmp_hdr->un.echo.sequence, ip_hdr->ttl);
		printf("time=%f \n", time_elapsed);
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	ip_hdr->ttl = 1;
	uint8_t type = ICMP_TIME_EXCEEDED;		/* message type */
	
	while (type == ICMP_TIME_EXCEEDED){
		...
		*/
		...
		icmp_hdr->code = 0;
		icmp_hdr->un.echo.id = getpid();
		icmp_hdr->un.echo.sequence = htons(13);
		icmp_hdr->checksum = 0;
		icmp_hdr->checksum = checksum(icmp_hdr, sizeof(struct icmphdr));
