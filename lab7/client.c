...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        int send_test = send(sockfd, buf, byte_count, 0);
        if (send_test <= 0){
            perror ("Nu am putut trimite - run_client");
        }
        memset(buf, 0, BUFLEN);
       
        // TODO 5: Receptionati un mesaj venit de la server
        int recv_test = recv(sockfd, buf, BUFLEN, 0);
        if (recv_test < 0){
            perror("Nu am primit nimic de la server - run_client");
        }
...
int main(int argc, char* argv[])
{
    ...
    if (c < 0)
        perror("Conexiunea nu s-a putut realiza");
