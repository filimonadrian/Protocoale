...
int receive_and_send(int connfd1, int connfd2)
{
    ...
    if (bytes_received < 0){
        perror("Server : Nu am putut receptiona mesaj");
    }
    if (bytes_received != 0) {
    ...
    if (bytes_send < 0){
        perror ("server: Nu pot trimite mesajul");
    }
...
void run_echo_server(int listenfd)
{
    ...
    if (l < 0){
        perror ("NU ascult conexiunea - run_echo_server");
    }

    // TODO 5: Acceptati o conexiune
    connfd = accept(listenfd, (struct sockaddr*) &client_addr, &socket_len);
    if (connfd < 0){
        perror ("Nu pot accepta o conexiune - run_echo_server");
    }
...
void run_chat_server(int listenfd)
{
    ...
    if (connfd1 < 0 || connfd2 < 0 ){
        perror ("Nu pot accepta o conexiune - chat-server");
    }
...
int main(int argc, char* argv[])
{
    ...
    if (b < 0){
        perror ("Nu am facut bind - server");
    }
